...
interface UserListProps {
  ...
  onlineUsers?: Set<string> // Add online users
}

const UserList = ({ users, selectedUser, onSelectUser, recentConversations, onlineUsers = new Set() }: UserListProps) => {
        ...
        return (
          ...
          >
            <div className="relative">
              ...
              <div className={`absolute bottom-0 right-2 w-3 h-3 rounded-full ${isOnline ? 'bg-green-500' : 'bg-gray-400'}`}></div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-baseline">
                <h3 className="text-sm font-medium text-gray-900 truncate">{user.name}</h3>
                <span className="text-xs text-gray-500">
                  {isOnline ? 'online' : (user.updatedAt ? formatTime(user.updatedAt) : 'offline')}
                </span>
              </div>
