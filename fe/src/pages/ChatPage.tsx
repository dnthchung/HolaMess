...
const ChatPage = () => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await axios.get(
          `/api/auth/users?exclude=${user.phone}`
  ...
  useEffect(() => {
    const fetchConversation = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `/api/messages/conversation/${user.id}/${selectedUser.id}`
        );
        console.log("Fetched conversation:", response.data);
        setMessages(response.data);

        await axios.put(
          `/api/messages/mark-read/${user.id}/${selectedUser.id}`
        );

        const recentsResponse = await axios.get(
          `/api/messages/recent/${user.id}`
        );
  ...
  const handleSendMessage = (content: string) => {
    ...
    socket.emit("private_message", messageData, (acknowledgement: any) => {
      ...
      if (acknowledgement && acknowledgement._id) {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg._id === tempId ? { ...msg, _id: acknowledgement._id } : msg
          )
        );
      }
    });
  ...
  const handleLogout = () => {
    ...
    disconnectSocket(); // Clear user data from storage
    localStorage.removeItem("user");
    sessionStorage.clear(); // Update user context
    setUser(null); // Redirect to login page
    navigate("/login");
  };

  useEffect(() => {
    if (!socket) return; // Listen for user online status
  ...
  }, [socket]); // Handle read receipts from other devices

  useEffect(() => {
    if (!socket) return;

    const handleMessagesRead = (data: { otherUserId: string }) => {
      console.log("Messages read on another device:", data); // If the read messages are for the current conversation, update the UI
    ...
    const handleReceiptRead = (data: {
      ...
      console.log("Receipt read:", data); // If this receipt is for the current conversation, update the messages
  ...
  }, [socket, selectedUser, user]); // Function to refresh conversation data

  const refreshConversation = async (userId: string, otherUserId: string) => {
    if (!userId || !otherUserId) return;
    try {
      console.log(
        "Refreshing conversation between",
        userId,
        "and",
        otherUserId
      );
  ...
  const handleInputFocus = async () => {
    ...
    try {
      ...
      await axios.put(
        `/api/messages/mark-read-focus/${user.id}/${selectedUser.id}`
      ...
      refreshRecentConversations(); // Notify via socket
      if (socket) {
  ...
  const formatUserStatus = (userId: string) => {
    ...
    if (conversation && conversation.lastMessage?.createdAt) {
      ...
      if (diffMs < 24 * 60 * 60 * 1000) {
        ...
        return (
          <span className="text-gray-500 text-sm">Active {hours}h ago</span>
        );
      }
